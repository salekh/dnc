...
description: Deploy the Ruby TV recommendations service to target environments, manage canaries, and configure rollbacks.
---

# Deploy Recommendations Service Skill

This skill allows agents to execute, monitor, and rollback deployments of the Ruby TV `recs-serving-service`.
...
rapid-cli cluster-status --cluster=rubytv-staging-cell-1
```
...
rapid-cli deploy \
  ...
  --image=gcr.io/ruby-tv/recs-service:v2.1.0-rc3 \
  --environment=staging \
  --canary_percent=5 \
  --timeout_seconds=300
```
